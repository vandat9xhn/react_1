...
// 
import { API_FashionProduct_L, API_FashionHotImage_L } from '../../../../api/api_django_no_token/fashion/APIFashionNoToken';
...
/* -------------------- GET API ------------------------ */
    ...
    getListHotImage = async () => {
        ...
        // 
        this.setState({
            hot_images: [hot_images[hot_images.length - 1], ...hot_images, hot_images[0]],
    ...
    // render
    render() {
        const {is_fetching, has_fetched, list, hot_images, count} = this.state
                ...
                <div className="Fashion_body">
                    <div>
                        <div className="Fashion_carousel">
                            <FashionBC images={has_fetched ? hot_images : [image_loading, image_loading, image_loading]}/>
                        </div>
                    ...
                    <div>
                        <ul className="Fashion__list">
                            {(has_fetched ? list : new Array(5).fill({vid_pics: []})).map((item, index) => {
                                const {id, vid_pics, name, new_price, old_price} = item;
                                return(
                    ...
                    <div className={count > list.length && has_fetched ? 'Fashion_more' : 'display-none'}>
